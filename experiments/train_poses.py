...
def options(argv=None):
    ...
    parser.add_argument('-i', '--base-dir', required=True, type=str,
                        metavar='PATH', help='path to the dataset base directory') # like '/path/to/ModelNet40'
    parser.add_argument('-p', '--partition', required=False, type=int, default=0.8,
                        metavar='N', help='portion of train/valid split') # like '/path/to/ModelNet40'
    parser.add_argument('-s', '--sequences', required=True, type=str, nargs='+',
                        metavar='N', help='list of sequences') # like '/path/to/ModelNet40'
    ...
    parser.add_argument('--num-points', default=4096, type=int,
                        metavar='N', help='points in point-cloud (default: 4096)')
...
class Action:
    ...
    def create_model(self):
        feat1 = ptlk.pointnet.PointNet_features(self.dim_k, self.use_tnet, self.sym_fn)
        # return ptlk.pointnet.PointNet_classifier(self.dof, feat, self.dim_k)
        feat2 = ptlk.pointnet.PointNet_features(self.dim_k, self.use_tnet, self.sym_fn)
        return ptlk.pointnet.PointNet_pose_regressor(self.dof, feat1, feat2, self.dim_k)
    ...
    def compute_loss(self, model, data, device):
        points_src, points_tar, rel_pose = data

        points_src = points_src.to(device)
        points_tar = points_tar.to(device)
        rel_pose = rel_pose.to(device)

        output = model(points_src, points_tar)
        loss = model.loss(output, rel_pose)

        return rel_pose, output, loss
...
def get_datasets(args):
    ...
    for seq in args.sequences:
        ...
        seq_poses.extend([data.poses[pose_i] for pose_i in range(data.poses.shape[0])])
    orig_dset = ptlk.data.datasets.KITTIVelo(seq_velo_files, seq_poses, args.num_points, args.partition)
